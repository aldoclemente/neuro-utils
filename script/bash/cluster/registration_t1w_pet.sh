...
for k in */; do
	...
	$APPTAINER exec $FSL fnirt --in=unwarped.nii.gz --aff=affine_transf.mat --cout=nonlinear_transf --config=$UTILSDIR/data/mni_icbm152_nlin_asym_09c/config
	$APPTAINER exec $FSL applywarp --ref=$MNI152T1 \
		  --in=unwarped.nii.gz --warp=nonlinear_transf --out=warped.nii.gz
$APPTAINER exec $FSL slice warped.nii.gz $MNI152T1 -o slices.png
    cd ..
done
cd .. # sub
